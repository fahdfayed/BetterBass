...
export default function HarmonyFretboard({embedded=false,centre:givenCentre,progression:givenProgression,livePitch,listening=false,homeMode,displayMode,fog,selectedPc,onSetRoot,onSetMode,onSetChord,onDisplayMode,onFog,onSelectPc,onAudition}:Props){
 ...
 return <div className="osScreen harmonyFretboard">
  ...
  <div className="hfBoardWrap"><div className="hfBoard" data-labels={actualDisplay} style={{minWidth:`${Math.max(520,visibleFrets.length*44+58)}px`}}><div className="hfFretNumbers" style={{gridTemplateColumns:`58px repeat(${visibleFrets.length}, minmax(44px, 1fr))`}}><b>{"String"}</b>{visibleFrets.map(f=><span className={[3,5,7,9,12,15,17,19].includes(f)?"marked":""} key={f}>{f}<i/></span>)}</div>{STRINGS.map((string,stringIndex)=><div className={`hfString string-${stringIndex}`} style={{gridTemplateColumns:`58px repeat(${visibleFrets.length}, minmax(44px, 1fr))`}} key={string.name}><b>{string.name}<small>{"String"}</small></b>{visibleFrets.map(fret=>{const pc=mod(string.open+fret),role=classifyNote(pc,current,selectedScale,next),show=visible(role),picked=selected===pc,destination=selectedDestination===pc;const under=heardAt.has(`${stringIndex}:${fret}`),sounding=heardPc===pc&&!under;return <button type="button" aria-pressed={picked} aria-label={`${string.name} string fret ${fret}: ${PITCH_NAMES[pc]}, ${role.label}${under?", playing now":""}`} onClick={()=>onSelectPc(pc)} className={`role-${role.id} ${show?"visible":"hidden"} ${picked?"picked":""} ${destination?"destination":""} ${under?"under":""} ${sounding?"sounding":""}`} key={fret}><i/><b dir="ltr">{filter===4?"?":cellText(pc,role)}</b><small dir="ltr">{filter===4?"":PITCH_NAMES[pc]}</small></button>})}</div>)}</div></div>
  </section>
