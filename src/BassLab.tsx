 ...
 {view==="courseLesson"&&<LessonWorkspace
  ...
  instruction={<>
   ...
   {courseStep===1&&<PacedReader className="hearStage" revealAll={courseIndex<courseCompleted}><article><span>HEAR BEFORE YOU PLAY</span><h2>Build an internal prediction.</h2><p>Listening is not passive exposure. Every round has a prediction, a sound and a verbal answer. Keep the bass muted until the imagined pitch feels specific.</p></article>
    ...
    {courseDetail.listening&&<section className="listeningList"><header><span>HEAR IT IN MUSIC SOMEBODY MADE</span><h2>The sound already exists. Go and find it.</h2><p>Listen once without an instrument. You are looking for the degree this lesson is about, not for the whole arrangement.</p></header><div>{courseDetail.listening.map(r=><article key={r.title+r.artist}><b>{r.title}</b><small>{r.artist}</small><p>{r.hear}</p></article>)}</div></section>}
   ...
   {courseStep===3&&<PacedReader className="practiceStage" revealAll={courseIndex<courseCompleted}><header><span>GUIDED PRACTICE</span><h2>Exact work. Measurable passes.</h2><p>Practice is ordered from preparation to controlled execution to musical use. Advance only after three clean repetitions—not after one lucky attempt.</p></header><aside className="questLaunch"><div><span>PROVE IT ON THE INSTRUMENT</span><b>Walk this lesson one note at a time.</b><p>The path asks for a note, listens, and does not move until it hears the right one. A wrong turn costs ground back to the last place worth standing.</p></div><button type="button" className="action action-primary" onClick={()=>setView("quest")}>Open the walk <span className="arrow" aria-hidden="true">→</span></button></aside>
