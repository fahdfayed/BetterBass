...
import {
 AXES,type Axis,type TakeEvent,type ExerciseStep,type DetectorSpec,type CoachBlock,type BlockAnalysis,
 NOTES,clamp,pc,noteAt,timeline,defaultDetector,getSteps,getDetector,analyzeEvents,axisBlueprint,scoringAxisFor,
} from "./coach-scoring";
 ...
 useEffect(()=>{
  ...
  const finishStage=(runtime:RunnerRuntime,block:CoachBlock,analysis:BlockAnalysis,repaired:boolean,now:number)=>{
   ...
   setData(previous=>{
    ...
    let attempts=previous.attempts;
    const proofAxis=scoringAxisFor(block);
    if(proofAxis){
